...
while (k<N+1){
  ...
  underprof <- pi2(theta1)*df (theta1 %>% pi2) *df
  underprof <- underprof %>% round(,digits=3)
  ...
  overprof = (theta1 %>% pi1)*df cbind((theta1 %>% pi2)*df - cost1) %>% round(digits=3)
  
  prof <- c(underprof,c(t(overprof)))
  
  upper0_label <- paste("P(",rep(0,length=TH),",",seq(1:TH),")",sep="")  
  upper1_label <- paste("P(",rep(1,length=TH),",",seq(1:TH),") - C",seq(1:TH),sep="")
  ...
  lower_label <- paste("P(",rep(1,length=TH-1),",",seq(2,TH),")",sep="")
  ...
  EM_Path_matrix[k,] <- get.shortest.paths(adjg,from=TH+1,to=2*TH+1,weights=prof)$vpath[[1]]
...
plot(Adopt_prop,type='l',ylim=c(0,1),xlab='Time (in Periods)',ylab='Cumulative Adoption (Percentage)',lwd=1.5)
...
eligible_theta_iteration <- eligible_theta[! eligible_theta_Index %in% 0]
...
while(k < N0+1){
  ...
  overprof <- array(0,dim=c(TH,2))
  overprof[,1] <- pi1(theta1)*df
  overprof[,2] <- pi2(theta1)*df-(funccost(time)-subsidy*prob)*df
  overprof <- round(overprof,digits=3)
  ...
  EN_Path_matrix[k,] <- get.shortest.paths(adjg,from=TH+1,to=2*TH+1,weights=prof)$vpath[[1]]
...
INDEX <- 1:N0
eligible_Index_Iteration <-eligible_theta_Index[! eligible_theta_Index %in% 0]

for(i in announce:(A-1)){
  
  eligible_theta[1:eligible_Index_Iteration[EN_Adopt_prop[i]]] <- 10000
...
while(q < TH+1){
  ...
  while(k<N0+1){
    ...
    overprof[,1] <- pi1(theta1)*df
    overprof[,2] <- pi2(theta1)*df-(funccost(time)-subsidy*prob)*df
    overprof[q,2] <- pi2(theta1)*df[q]-(funccost(time[q])-subsidy)*df[q]
    overprof[1:(q-1),2] <- pi2(theta1)*df[1:(q-1)]-(funccost(time[1:(q-1)]))*df[1:(q-1)]
    overprof <- round(overprof,digits=3)
    ...
    EP_Path_matrix[k,] <- get.shortest.paths(adjg,from=TH+1,to=2*TH+1,weights=prof)$vpath[[1]]
  ...
  if (N0 != 0){
    ...
    while(k<N0+1){
      ...
      overprof[,1] <- pi1(theta1)*df
      overprof[,2] <- pi2(theta1)*df-(funccost(time)-subsidy*prob)*df
      overprof[1:q,2] <- pi2(theta1)*df[1:q]-(funccost(time[1:q]))*df[1:q]
      overprof <- round(overprof,digits=3)
      ...
      EG_Path_matrix[k,] <- get.shortest.paths(adjg,from=TH+1,to=2*TH+1,weights=prof)$vpath[[1]]
    ...
    if(round(Budget/subsidy,digits=0) < Pos_Index){
      Index_Offer <- sample(1:Pos_Index,round(Budget/subsidy,digits=0),0)
    ...
    ######@PROBLEM AREA@#########
    eligible_Index_Iteration <- eligible_Index_Iteration[! eligible_Index_Iteration %in% 0]
    eligible_theta_Index <- rep(0,times=N)
    for(i in 1:length(eligible_Index_Iteration)){
      eligible_theta_Index[eligible_Index_Iteration[i]] <- eligible_Index_Iteration[i]  
...
Pos_Index
prob
plot(Adopt_prop,type='l',main=paste("Budget = ",Budget," Subsidy = ",round(subsidy,digits=2),"\n\ Active Period = ", A, " Announcement Period = ",announce),ylim=c(0,1)
...
legend(3,.9,pch=c(1,3),col=c("black","blue"),legend=c("Adoption Under Neutral Expectations","Simulated Adoption"))
...
plot(time*pi1(theta[600])-(time*pi2(theta[600])-funccost(time)),type='l')
time*pi2(theta[600]-funccost(time))
change <- c()
for (i in 1:length(time)-1){
  change[i] <- time[i+1]*pi2(theta[600]-funccost(time[i+1]))-time[i]*pi2(theta[600]+funccost(time[i]))
}
